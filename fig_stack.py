...
# WHO RELAYS: PIVOT backbone + pressure fallback (bottom left)
box(0.35, 0.12, 4.1, 2.28, "WHO RELAYS \u2014 PIVOT backbone  (\u00a76.5, \u00a76.9)", [
    "\u2022 local two-hop MPR: mandatory + greedy pivots",
    "\u2022 leases + utilization load-balance hubs",
    "\u2022 non-pivots wait for echo; backup if silent",
    "\u2022 64-bit digest in header (~8 B); trades reach",
    "   for capacity on one price curve, not a new one",
    "\u2022 pressure (crowding+novelty) = per-flood fallback",
], "#006d2c", title_fs=8.8)
link(2.6, 2.40, 4.2, 4.35, "#006d2c")

# GOVERNORS (bottom right)
box(5.0, 0.1, 4.75, 2.15, "HOW MUCH / HOW FAST \u2014 governors  (\u00a76.7, \u00a76.10)", [
...
GAINS = [
    ...
    ("ASSURED RELIABILITY ENVELOPE", DGREEN,
     "25.3%  \u2192  94.5%", "optional reliability mode \u2014 not the network-wide default"),
    ("DELIVERY AT 200 HOPS", GREEN,
     "0%  \u2192  97%", "no hop limit + per-hop persistence, linear cost"),
    ("NODE CAPACITY (Norway)", "#006d2c",
     "445  \u2192  634 hotspot nodes", "+42% vs managed flood at same utilization cap"),
...
for title, color, delta, note in GAINS:
    axb.add_patch(FancyBboxPatch((0.25, y - 0.96), 9.5, 1.0,
                                 boxstyle="round,pad=0.05,rounding_size=0.10",
                                 fc="white", ec=color, lw=1.6))
    axb.text(0.5, y - 0.06, title, fontsize=7.7, fontweight="bold", color=color,
             va="top")
    axb.text(0.5, y - 0.36, delta, fontsize=10.0, fontweight="bold",
             color="#111", va="top")
    axb.text(0.5, y - 0.72, note, fontsize=7.2, color="#444", va="top")
    y -= 1.16
